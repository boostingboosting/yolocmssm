...
class v8DetectionLoss:
    ...
    def compute_loss(self, preds, batch):
        ...
        # for i, offset in enumerate(batch['offset'][:16]):
        #     print(f"第{i}个元素: 类型={type(offset)}, 内容={offset}, 形状（若数组）={offset.shape if isinstance(offset, np.ndarray) else '非数组'}")
        # print("batch['cls_rgb']:", batch['cls_rgb'])
        # offset_np = np.stack([offset.reshape(4) if offset.shape[0] !=0 else np.zeros(4, dtype=np.float32) for offset in batch['offset']])
        # batch['offset'] = torch.from_numpy(offset_np).to('cuda')
        ...
        ## offset loss
        # print("self.hyp.use_offset333333333333:", self.hyp.use_offset)
        # print("batch['offset'][:,:2]333333333333:", batch['offset'][:,:2])
        # print("len(txty_pred)s3333333333333:", len(txty_preds))
        # print(" batch['area']333333333333:",  batch['area'])
        # print("preds[0]333333333333:",preds[0])
        if self.hyp.use_offset:
    ...
    def compute_offset_loss(self, txtys, targets, area_ir):
        ...
        offset_gt = targets.reshape(-1, 2).to(device)  # [16,2]
